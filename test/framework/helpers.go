...
func (f *Framework) GetMetricVal(ctx context.Context, ns, podName, portNumberOrName, metricName, scheme string) (float64, error) {
	resourceName := podName
	if portNumberOrName != "" {
		resourceName = fmt.Sprintf("%s:%s:%s", scheme, podName, portNumberOrName)
	}
